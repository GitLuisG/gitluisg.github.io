...
function postGaleria(img, color = "white", width="") {
    return (
        '<div class="row mb-3"> <div class="col-sm-6 bg-'+color+'"> <img width="'+width+'" class="img-fluid" src="' +
        img[0] +
...
function getList(List){
  var c = `<ol class="list-group list-group-numbered">`;
    for (let index = 0; index < List.length; index++) {
        c += '<li class="list-group-item">' + List[index] + "</li>";
    }
    c+= '</ol>';
  return c;
}

function getArticle(Art, List, index){
  var c = `<ol class="list-group list-group-numbered">`;
      c+= `<div class="accordion accordion-flush" id="accordionFlushExample">
                    <div class="accordion-item">
                      <h2 class="accordion-header">
                        <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseThree`+index+`" aria-expanded="false" aria-controls="flush-collapseThree">
                          `+Art+`
                        </button>
                      </h2>
                      <div id="flush-collapseThree`+index+`" class="accordion-collapse collapse" data-bs-parent="#accordionFlushExample">
                        <div class="accordion-body">`+getList(List)+`</div>
                      </div>
                    </div>
                  </div>`;
    c+= '</ol>';
